...
class CycleReport:
    ...
    **Always None on a daily report.** Task 4.6 asked what the daily path puts
    ...
    minute_trailing_required: bool = True
    """False when the minute pass ran on a day ``MT_MINUTE_FIRING_DAYS`` does
    not name: no current session was owed, the trailing phase was skipped and
    the whole allowance went to backfill, so ``minute_trailing_completed`` is
