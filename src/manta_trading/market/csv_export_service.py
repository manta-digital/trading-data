...
class CSVExportService:
    ...
    def __init__(self, db: TimescaleMinuteDataDB, export_base_dir: str = "/data/exports"):
        """
        ...
        _logger.info("CSV export service initialized with base directory: %s", export_base_dir)
        
    async def export_symbol_to_csv(self, symbol: str, start_date: datetime, 
                                  end_date: datetime, export_format: str = 'daily') -> Dict:
        """
        ...
        try:
            ...
            if export_format == 'daily':
                await self._export_daily_files(symbol, start_date, end_date, symbol_dir, export_stats)
            elif export_format == 'monthly':
                await self._export_monthly_files(symbol, start_date, end_date, symbol_dir, export_stats)
            elif export_format == 'single':
                await self._export_single_file(symbol, start_date, end_date, symbol_dir, export_stats)
            else:
    ...
    async def _export_daily_files(self, symbol: str, start_date: datetime, 
                                 end_date: datetime, symbol_dir: Path, stats: Dict):
        ...
        while current_date <= end_date_only:
            ...
            if not day_data.empty:
                ...
                _logger.debug("Exported %d rows for %s on %s", len(day_data), symbol, current_date)
            
            current_date += timedelta(days=1)
    
    async def _export_monthly_files(self, symbol: str, start_date: datetime, 
                                   end_date: datetime, symbol_dir: Path, stats: Dict):
        ...
        while current_month <= end_date:
            ...
            if not month_data.empty:
                ...
                _logger.debug("Exported %d rows for %s in %s", len(month_data), symbol, current_month.strftime('%Y-%m'))
    ...
    async def _export_single_file(self, symbol: str, start_date: datetime, 
                                 end_date: datetime, symbol_dir: Path, stats: Dict):
        ...
        if not data.empty:
            file_path = symbol_dir / f"{symbol}_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.csv"
    ...
    async def export_aggregated_bars(self, symbol: str, start_date: datetime, 
                                    end_date: datetime, aggregations: list[str] = None) -> dict:
        """
        ...
        try:
            ...
            for agg in aggregations:
                try:
                    agg_data = await self.db.get_minute_data(symbol, start_date, end_date, aggregation=agg)
                    ...
                    if not agg_data.empty:
                        ...
                        _logger.debug("Exported %d %s bars for %s", len(agg_data), agg, symbol)
                    else:
            ...
            _logger.info("Aggregated bars export completed for %s: %s", symbol, export_stats)
    ...
    async def export_bulk_symbols(self, symbols: list[str], start_date: datetime,
                                 end_date: datetime, export_format: str = 'monthly',
        ...
        try:
            ...
            async def export_symbol(symbol: str):
                try:
                    return await self.export_symbol_to_csv(symbol, start_date, end_date, export_format)
                except Exception as e:
            ...
            total_files = sum(r.get('files_created', 0) for r in results.values() if 'error' not in r)
