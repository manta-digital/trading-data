...
class FetchInstruction:
    ...
    def __str__(self) -> str:
        chunk_info = (
            f"({self.chunk_number}/{self.total_chunks})"
...
class ChunkingStrategy(ABC):
    ...
    def determine_fetch_plan(
        self, symbol: str, last_updated: Optional[date]
    ) -> list[FetchInstruction]:
        """
...
class DefaultChunkingStrategy(ChunkingStrategy):
    ...
    def determine_fetch_plan(
        ...
        if days_since_update < self.config.MIN_DAYS_THRESHOLD:
            _logger.debug(
                "Skipping %s: updated %d days ago (< %d)",
                symbol,
                days_since_update,
                self.config.MIN_DAYS_THRESHOLD,
            )
            return []
        ...
        _logger.debug(
            "DefaultStrategy for %s: %d days -> %s",
            symbol,
            days_since_update,
            output_size,
        )
        ...
        return [
            FetchInstruction(
                symbol=symbol, output_size=output_size, chunk_number=1, total_chunks=1
            )
        ]
...
class CompactChunkingStrategy(ChunkingStrategy):
    ...
    def determine_fetch_plan(
        ...
        _logger.debug(
            "CompactStrategy for %s: %d days -> compact", symbol, days_since_update
        )

        return [
            FetchInstruction(
                symbol=symbol, output_size="compact", chunk_number=1, total_chunks=1
            )
        ]
...
class IntelligentChunkingStrategy(ChunkingStrategy):
    ...
    def determine_fetch_plan(
        ...
        if last_updated is None or days_since_update > (
            self.config.CHUNK_SIZE_DAYS * 20
        ):  # > 2000 days
            _logger.debug(
                "Full fetch for %s: %s",
                symbol,
                "new symbol"
                if last_updated is None
                else f"{days_since_update} days old",
            )
            return [
                FetchInstruction(
                    symbol=symbol, output_size="full", chunk_number=1, total_chunks=1
                )
            ]
        ...
        if days_since_update <= self.config.RECENT_DAYS_THRESHOLD:
            _logger.debug(
                "Compact fetch for %s: %d days gap", symbol, days_since_update
            )
            return [
                FetchInstruction(
                    symbol=symbol, output_size="compact", chunk_number=1, total_chunks=1
    ...
    def _create_chunk_plan(self, symbol: str, days_gap: int) -> list[FetchInstruction]:
        ...
        chunks_needed = (
            days_gap + self.config.CHUNK_SIZE_DAYS - 1
        ) // self.config.CHUNK_SIZE_DAYS
        ...
        if chunks_needed > max_chunks:
            _logger.warning(
                "Gap too large for chunking (%d chunks), using full fetch for %s",
                chunks_needed,
                symbol,
            )
            return [
        ...
        _logger.debug(
            "Created %d chunk plan for %s (%d day gap)", chunks_needed, symbol, days_gap
        )
        return instructions
...
class ChunkingStrategyFactory:
    ...
    def get_intelligent_strategy(
        config: Optional[ChunkingConfig] = None, gap_analyzer=None
    ) -> ChunkingStrategy:
