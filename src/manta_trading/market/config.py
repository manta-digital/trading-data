...
class ChunkingConfig:
    ...
    def __init__(self):
        ...
        self.MAX_LOOKBACK_DAYS = int(os.getenv('MAX_LOOKBACK_DAYS', '200'))  # For gap detection
    ...
    def _validate_config(self):
        ...
        if self.CHUNK_SIZE_DAYS <= 0:
            raise ValueError(f"CHUNK_SIZE_DAYS must be positive, got {self.CHUNK_SIZE_DAYS}")
            
        if self.MIN_DAYS_THRESHOLD < 0:
            raise ValueError(f"MIN_DAYS_THRESHOLD must be non-negative, got {self.MIN_DAYS_THRESHOLD}")
            
        if self.MAX_ERROR_COUNT <= 0:
            raise ValueError(f"MAX_ERROR_COUNT must be positive, got {self.MAX_ERROR_COUNT}")
            
        if self.RECENT_DAYS_THRESHOLD <= 0:
            raise ValueError(f"RECENT_DAYS_THRESHOLD must be positive, got {self.RECENT_DAYS_THRESHOLD}")
            
        if self.MAX_LOOKBACK_DAYS <= 0:
            raise ValueError(f"MAX_LOOKBACK_DAYS must be positive, got {self.MAX_LOOKBACK_DAYS}")
            
        if self.CHUNK_DELAY_SECONDS < 0:
            raise ValueError(f"CHUNK_DELAY_SECONDS must be non-negative, got {self.CHUNK_DELAY_SECONDS}")
        ...
        _logger.debug(
            ...
            self.CHUNK_SIZE_DAYS, self.MIN_DAYS_THRESHOLD, self.RECENT_DAYS_THRESHOLD
        )
