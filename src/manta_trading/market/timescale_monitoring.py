...
class TimescaleMonitor:
    ...
    def __init__(self, enable_performance_logging: bool = True, enable_alerts: bool = True):
        """
        ...
        _logger.info("TimescaleMonitor initialized with performance_logging=%s, alerts=%s",
                   enable_performance_logging, enable_alerts)
    ...
    async def monitor_operation(self, operation: str, symbol: str = "N/A"):
        ...
        try:
            ...
            if metrics.rows_processed > 0 and metrics.duration_seconds > 0:
                metrics.rows_per_second = metrics.rows_processed / metrics.duration_seconds
    ...
    async def _log_performance_metrics(self, metrics: PerformanceMetrics):
        ...
        log_data = {
            'event_type': 'timescale_performance',
            'metrics': metrics.to_dict()
        }

        if metrics.error:
            _logger.error("TimescaleDB operation failed: %s", json.dumps(log_data, indent=2))
        elif metrics.duration_seconds > 10:  # Log slow operations as warnings
            _logger.warning("TimescaleDB slow operation: %s", json.dumps(log_data, indent=2))
        else:
            _logger.info("TimescaleDB operation completed: %s in %.3fs (%d rows, %.0f rows/s)",
                       metrics.operation, metrics.duration_seconds,
                       metrics.rows_processed, metrics.rows_per_second)
    ...
    async def _check_performance_alerts(self, metrics: PerformanceMetrics):
        ...
        if (metrics.operation == 'bulk_write' and
            metrics.rows_per_second > 0 and
            metrics.rows_per_second < self.performance_thresholds['bulk_write_min_rows_per_sec']):
            ...
            await self._generate_alert(
                ...
                f"Bulk write performance below threshold: {metrics.rows_per_second:.0f} rows/s",
                {
                    ...
                    'threshold': self.performance_thresholds['bulk_write_min_rows_per_sec'],
                    'duration': metrics.duration_seconds
                }
            )

        # Check query duration
        if (metrics.operation in ['query', 'aggregation'] and
            metrics.duration_seconds > self.performance_thresholds['query_max_duration_sec']):
    ...
    async def _generate_alert(self, alert_type: str, severity: str, message: str, details: dict[str, Any]):
        ...
        log_data = {
            'event_type': 'timescale_alert',
            'alert': alert.to_dict()
        }

        if severity == 'critical':
            _logger.critical("TimescaleDB CRITICAL ALERT: %s", json.dumps(log_data, indent=2))
        elif severity == 'error':
            _logger.error("TimescaleDB ERROR ALERT: %s", json.dumps(log_data, indent=2))
        elif severity == 'warning':
            _logger.warning("TimescaleDB WARNING ALERT: %s", json.dumps(log_data, indent=2))
        else:
    ...
    async def log_system_health(self, health_data: dict[str, Any]):
        ...
        if overall_status == 'error':
            _logger.error("TimescaleDB system health check failed: %s", json.dumps(log_data, indent=2))
        elif overall_status == 'unhealthy':
            _logger.warning("TimescaleDB system health issues detected: %s", json.dumps(log_data, indent=2))
        else:
            _logger.info("TimescaleDB system health check: status=%s, symbols=%s, total_rows=%s",
                       overall_status,
                       health_data.get('coverage_summary', {}).get('symbols_tracked', 0),
                       health_data.get('coverage_summary', {}).get('total_rows', 0))

    async def log_compression_event(self, symbol: str, before_size: int, after_size: int,
        ...
        _logger.info("TimescaleDB compression completed for %s: %.1f%% savings (%.1fMB) in %.1fs",
                   symbol, compression_ratio * 100, savings_mb, duration)
    ...
    def set_performance_threshold(self, metric: str, value: float):
        ...
        if metric in self.performance_thresholds:
            ...
            _logger.info("Updated performance threshold: %s=%s (was %s)", metric, value, old_value)
        else:
...
def configure_monitor(enable_performance_logging: bool = True, enable_alerts: bool = True):
