...
3. [ ] **Near-Real-Time Collection** — To use Kalshi data in live trading, collection must move closer to real time than the sparse-timer pass provides: a materially shorter pass cadence for selected surfaces, or streaming (websocket) subscriptions per the production-form ADR's `Type=simple` reservation. Requires its own scoping — market selection, latency target, and interaction with the cross-source arbitration gap (a tight cadence stops being "low-priority background" by construction). Dependencies: [264, 265].
