...
- [x] Add `test/integration/test_symbol_ranges_sql.py` (D2)
  - [x] Against an ephemeral DB seeded with `minute_coverage`/`daily_coverage`
        ...
        one symbol each
  - [x] Execute the three real SQL statements from `queries.py` (not mocks) and
        assert the merged `(start, end)` per family matches a hand-computed
        expectation for every case
  - [x] Follow `test_data_status_equivalence.py`'s precedent for
        ...
        claim)
  - [x] Success: tests pass against `MT_TIMESCALE_TEST_URL`; no test depends on
        production data
  - [x] Effort: 3
...
- [x] Create `test/load/conftest.py` and extract `prod_shaped_db` (D10)
  - [x] Move `_symbol`, `_seed_prod_shape`, `_refresh_coverage`, and the
...
- [x] Create `test/load/test_187_api_nfr.py` and add assertions 1 and 4 — both are single-request latency bounds on the same app instance, so they share setup
  - [x] `pytestmark` matches slice 167's convention: `skipif` on
  ...
  - [x] Assertion 1 (D10.1): symbol-detail latency via `httpx.ASGITransport`
        ...
        treated as final
  - [x] Assertion 4 (D10.4): `GET /api/v1/status` against `prod_shaped_db`;
        provisional bound `< 1.5 s`, reusing slice 167's sub-second DB-side
        NFR plus serialization headroom
  - [x] Success: both assertions pass against `prod_shaped_db`
  - [x] Effort: 3
...
- [x] Add assertion 2 to `test_187_api_nfr.py`
  - [x] Uses the dense-minute fixture (Task 11); a `1m` request at exactly
        `MT_API_MAX_BARS_PER_REQUEST` bars
  - [x] Provisional bound `< 15 s`; assert the measured wall clock exceeds the
        ...
        point of the test
  - [x] Success: passes against the dense-minute fixture; module docstring
...
- [x] Add assertion 3 to `test_187_api_nfr.py`
  - [x] 16 concurrent symbol-detail requests against the app's real pool
        (`max_size=8`); all complete; none exceeds assertion 1's single-request
        bound by more than a stated queueing factor
  - [x] Record the measured per-request latencies at concurrency 16 — this
        output is Task 13's input, not just a pass/fail
  - [x] Success: passes against `prod_shaped_db`; latencies captured for Task 13
  - [x] Effort: 3
...
- [x] Finish `test_187_api_nfr.py`'s gating and documentation, and run the full tier
  - [x] Module docstring documents the manual gate command
        (`MT_RUN_LOAD_TESTS=1 uv run pytest test/load/`) and states CI wiring
        is slice 907's deliverable, not this slice's (D9)
  - [x] Success: `MT_RUN_LOAD_TESTS=1 uv run pytest test/load/` runs all four
...
- [x] Record the pool/consolidation decision from Task 12c's assertion-3 measurement
  - [x] Using assertion 3's captured latencies, decide whether the three-pool
        arrangement (`app.state.db_pool` + the two class-owned DB pools) costs
        something real at concurrency 16 against `max_size=8`
  - [x] Write the decision into design D11 (or a short addendum section) with
  ...
  - [x] If consolidation is warranted, implement it in this slice: success
        criterion 8 requires the decision be recorded either way, but if the
        numbers demand a change, D11 says it lands here
  - [x] Success: design document's D11 section (or an appended note) states
...
- [x] `180-arch.data-serving.md` — Symbol Detail section
  - [x] Replace the paragraph attributing the ranges cost to
        `minute_5min_ohlcv` and calling both queries "sub-millisecond index
        seeks" with the D1 measurement and the D2 read path
  - [x] Success: section matches what was actually built in file 1's Tasks 6–9
  - [x] Effort: 1

- [x] `180-slices.data-serving-api.md`, entry 7 — verification only, no implementation
  - [x] The `(187)` index and the corrected scope text (no guard-based
  ...
  - [x] Success: entry text matches the shipped code; no unexplained diff
  - [x] Effort: 1

- [x] `README.md` — `available` semantics
  - [x] Document: leading edge is exact (head probe), start is as of the last
        coverage materialization (D4), and the D3 residual window (quantified
        in Task 15's walkthrough step 4)
  - [x] Success: a client dev can learn the actual contract, including its one
        documented gap, from the README alone
  - [x] Effort: 1

- [x] `CHANGELOG.md`
  - [x] Entry under `[Unreleased]`: the `available` computation change and its
  ...
  - [x] Note the coverage-staleness flip is expected and tracked as slice 169
  - [x] Success: entry names all three user-visible effects
  - [x] Effort: 1

- [x] Regenerate `docs/api/openapi.json`
  - [x] `uv run python scripts/dump_openapi.py`; confirm the drift test
        (`test_openapi_artifact.py`, slice 186) shows no diff for the symbols
        route — response shape is unchanged (D2)
  - [x] Success: drift test passes; diff (if any) touches only `info.version`
  - [x] Effort: 1
...
- [x] Run the design's Verification Walkthrough steps 1–3, 5, 9 against prod `trading`
  - [x] Step 1: before/after latency on `GET /api/v1/symbols/{SPY,AAPL,F,<delisted>}`,
        five calls each, medians reported against the D1 baseline
  - [x] Step 2: `EXPLAIN (ANALYZE, BUFFERS)` each of the three new statements
        with production parameters; confirm single-digit-ms planning and chunk
        exclusion (success criterion 1 evidence)
  - [x] Step 3: merged vs. lazy `MIN/MAX` equivalence across 20+ symbols
        (dense, sparse, delisted, minute-only, daily-only, no-data) — success
        criterion 2
  - [x] Step 5: `check_coverage_freshness` against prod reports stale with
  ...
  - [x] Step 9: `/api/v1/symbols/SPY` response body diffed against a pre-slice
        capture — identical apart from the D2-corrected values; OpenAPI drift
        test green
  - [x] Success: each step's actual output recorded for the walkthrough rewrite
        below; every `statement_timeout` set explicitly per project convention
  - [x] Effort: 3

- [x] Run step 4: quantify the D3 residual window (load-bearing — success criterion 2's caveat clause)
  - [x] Across the same 20+ symbol sample, compute each symbol's own coverage
        end against the universe edge and check whether any raw bar falls in
        the gap
  - [x] Report the count and largest discrepancy
  - [x] **If the gap is not negligible**, apply the documented fallback before
        ...
        gap is negligible
  - [x] Success: finding recorded in the walkthrough; fallback applied only if
        warranted, with its own test coverage added under file 1 Task 7's file
        if so
  - [x] Effort: 2

- [x] Run steps 6–8: load tier against prod-shaped measurements
  - [x] Step 6: the detection-floor test (file 1 Task 3) shown failing when
        `_raw_max`'s alignment is removed, passing when restored
  - [x] Step 7: `MT_RUN_LOAD_TESTS=1 uv run pytest test/load/` end to end;
        record each assertion's measured median and the bound derived from it
  - [x] Step 8: assertion 3's per-request latencies at concurrency 16 reported
        alongside the Task 13 D11 decision
  - [x] Success: all measurements recorded for the walkthrough rewrite
  - [x] Effort: 2

- [x] Rewrite the design's Verification Walkthrough section
  - [x] Replace the draft walkthrough with the actual commands run and
  ...
  - [x] Do not revise decisions D1–D12 here beyond the bound corrections D10
        explicitly calls for
  - [x] Success: walkthrough section reflects reality; no other section edited
  - [x] Effort: 2
...
- [x] Full verification
  - [x] `uv run pytest test/unit test/integration -q` — compare pass count and
        error list against file 1 Task 1's baseline
  - [x] `MT_RUN_LOAD_TESTS=1 uv run pytest test/load/` green
  - [x] `uv run --extra dev mypy src/manta_trading/api_server/ src/manta_trading/data/maintenance/status_coverage.py src/manta_trading/market/maintenance/cagg_freshness.py` clean;
        `ruff` clean on every touched file
  - [x] `docs/api/openapi.json` regenerated as the final step so it reflects
        the merged state
  - [x] `cf check` clean
  - [x] Success: suite green, static analysis clean on touched files, `cf check`
        clean
  - [x] Effort: 1
