...
- [x] **T1 — Promote `_group_into_ranges` to a shared helper (pure refactor)**
  - [x] In [`compute_missing_ranges.py`](../../../src/manta_trading/data/gaps/compute_missing_ranges.py),
  ...
  - [x] Update the one internal call site in `compute_missing_ranges` to the new name.
  - [x] Export it as appropriate for reuse (e.g. add to `data/gaps/__init__.py`
    if that is the established import surface — match the existing pattern).
  - [x] Success: no behavior change to the daily path; grouping logic is now a
    single shared function (DRY).

- [x] **T2 — Verify the refactor (daily path unchanged)**
  - [x] Run `uv run pytest test/unit/data/gaps/test_compute_missing_ranges.py -q`
  - [x] Expected: all existing `compute_missing_ranges` tests pass unmodified.
  - [x] Run `uv run pyright src/manta_trading/data/gaps/compute_missing_ranges.py`;
    zero new errors.
  - [x] **Commit**: `refactor(gaps): extract group_sessions_into_ranges shared helper`

- [x] **T3 — Add the coverage-index statement-timeout constant**
  - [x] In [`constants.py`](../../../src/manta_trading/constants.py) add a named
  ...
  - [x] Success: constant is defined, typed, and documented; `pyright` clean.

- [x] **T4 — Implement `build_minute_coverage_index` in new `data/gaps/minute_coverage.py`**
  - [x] Create `src/manta_trading/data/gaps/minute_coverage.py`.
  - [x] `build_minute_coverage_index(conn) -> dict[str, set[date]] | None`:
  ...
  - [x] Success: function returns a populated dict on success, `None` on query
    failure; cagg name is sourced from `GRANULARITY_SOURCE`; parameterized/`SET
    LOCAL` SQL only; `pyright` clean.

- [x] **T5 — Implement `compute_missing_minute_sessions` in `data/gaps/minute_coverage.py`**
  - [x] `compute_missing_minute_sessions(conn, symbol, coverage_index, from_ts, to_ts) -> list[GapRange]`:
  ...
  - [x] Success: returns only-missing ranges; empty list for a fully-covered
    symbol; full-history ranges for an empty symbol; respects the delisted clamp;
    `pyright` clean.

- [x] **T6 — Unit tests for `minute_coverage.py`**
  - [x] Create `test/unit/data/gaps/test_minute_coverage.py`.
  - [x] `build_minute_coverage_index`: shape/grouping correctness against a mocked
  ...
  - [x] `compute_missing_minute_sessions`, parametrized cases:
  ...
  - [x] Mock the DB I/O boundary; test the diff/grouping logic with real data
    (per project testing rules).
  - [x] Run `uv run pytest test/unit/data/gaps/test_minute_coverage.py -q`; all pass.
  - [x] **Commit**: `feat(gaps): add coverage-aware minute session diff`

- [x] **T7 — Add `precomputed_ranges` path to `update_data_gaps`**
  - [x] In [`update_data_gaps.py`](../../../src/manta_trading/data/gaps/update_data_gaps.py),
    add an optional `precomputed_ranges: list[GapRange] | None = None` parameter.
  - [x] When provided (minute path), skip the single-span short-circuit at
  ...
  - [x] When `None` (daily path and any legacy caller), behavior is **identical**
    to today — no change to the daily `compute_missing_ranges` branch.
  - [x] Success: additive parameter; daily and `mt data refetch` call sites
    unaffected; `pyright` clean.

- [x] **T8 — Unit tests for the `precomputed_ranges` path**
  - [x] Extend `test/unit/data/gaps/test_update_data_gaps.py`:
  ...
  - [x] Run `uv run pytest test/unit/data/gaps/test_update_data_gaps.py -q`; all pass.
  - [x] **Commit**: `feat(gaps): update_data_gaps accepts precomputed minute ranges`

- [x] **T9 — Wire the seeder into the daemon + seed-phase progress + fail-safe**
  - [x] In [`daemon/minute.py`](../../../src/manta_trading/data/acquisition/daemon/minute.py):
  ...
  - [x] Seed-phase progress: accumulate `symbols scanned` and `gap rows seeded`
    ...
    by `mt data pull`).
  - [x] Success: covered symbols seed zero rows; missing sessions seed only their
...
- [x] **T10 — Daemon-level tests: coverage-aware seed + fail-safe**
  - [x] Add tests under `test/unit/data/acquisition/daemon/` covering:
  ...
  - [x] Run `uv run pytest test/unit/data/acquisition/daemon/ -q`; all pass.
  - [x] **Commit**: `feat(daemon): coverage-aware minute seeding with fail-safe`

- [x] **T11 — `_has_any_gaps` re-fire regression test**
  - [x] Add a regression test under `test/unit/data/acquisition/daemon/` pinning
  ...
  - [x] Reference design §Carry-forward / re-fire correctness and Success
    Criterion 4.
  - [x] Run the daemon test module; the new test passes.
  - [x] **Commit**: `test(daemon): pin _has_any_gaps re-seed to real holes only`

- [x] **T12 — Full local test + static-analysis pass**
  - [x] `uv run pytest test/ -q` — all pass; zero new failures (daily path
    behavior unchanged).
  - [x] `uv run ruff check src/ test/` — clean.
  - [x] `uv run pyright src/ test/` — zero errors.
  - [x] A failure here is a **STOP** condition — do not proceed to production
    verification with a red suite.

- [x] **T13 — Remove the obsolete `MINUTE_HISTORY_MONTHS` NFR (doc-only)**
  - [x] In [`140-arch.data-quality-operations.md`](../architecture/140-arch.data-quality-operations.md),
  ...
  - [x] Do **not** change `_resolve_minute_history_start` or any code — this is a
    doc reconciliation only.
  - [x] Success: the arch doc no longer contradicts the shipped operator-floor model.
  - [x] **Commit**: `docs(arch): remove obsolete 24-month minute-history NFR`

- [x] **T14 — Operational-fix re-audit (confirming pass)**
  - [x] Walk the four fixes per design §Operational-Fix Re-Audit and confirm no
  ...
  - [x] Success: re-audit confirms three no-change verdicts and one test-covered
    verdict; note the outcome in the slice walkthrough. No code change expected.

- [x] **T15 — Production verification walkthrough (operator-assisted)**
  - [x] Run the slice-design Verification Walkthrough end to end against `trading`
  ...
  - [x] Fill the concrete symbols/counts/EXPLAIN back into the slice-design
    Verification Walkthrough so an external agent can replay it verbatim.
  - [x] **Do not restart the production minute daemon** as part of this task —
    that is a separate PM go/no-go after this slice merges.
  - [x] A walkthrough step that does not behave as described is a **STOP**
    condition — confer with the Project Manager before marking the slice complete.

- [x] **T16 — Final commit + slice closeout**
  - [x] Update the slice-design frontmatter `status` → `complete` and
    `dateUpdated`; ensure the Verification Walkthrough reflects captured output.
  - [x] Update `CHANGELOG.md` per `project-guides/templates/changelog-format.md`.
  - [x] Run `workflow_check` (or `cf check`) with fix; resolve any findings.
  - [x] **Commit**: `docs: close out slice 162 — coverage-aware minute gap-seeding`
