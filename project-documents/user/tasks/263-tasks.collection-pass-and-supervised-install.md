...
- [x] **Task 1.1: Create `data/kalshi/run_context.py`** (effort: 2)
  - [x] Frozen dataclass `KalshiRun(settings, client, conn, sink, run_id,
        clock)` — exactly the six fields the design names.
  - [x] `open_kalshi_run(settings, events_file: Path | None)` as an
  ...
  - [x] `KalshiCredentialError` and `PreflightError` propagate unchanged (the
  ...
  - [x] Success: module imports; no import from `manta_trading.cli`; the
        dataclass has no other fields.

- [x] **Task 1.2: Rewire `run_sync` onto the context** (effort: 2)
  - [x] `run_sync` becomes: preflight via a small shared helper that opens
        ...
        unchanged.
  - [x] The preflight-to-exit-1 mapping exists in exactly one place; the
        `ProviderError` / `OperationalError` catches around `sync.run` stay
        as they are.
  - [x] Success: `uv run pytest test/unit/cli/commands/test_data_kalshi.py
        test/unit/data/kalshi -q` green with no test edited.

- [x] **Task 1.3: Unit tests for the context** (effort: 1)
  - [x] In `test/unit/data/kalshi/test_run_context.py` (fakes patched at the
  ...
  - [x] In `test_data_kalshi.py`: the preflight-exit-1 assertion is stated
        **once** for the shared helper (design *Tests*), not per command.
  - [x] **Commit**: `refactor: extract KalshiRun preflight context from run_sync`.

## Section 2: Phase contract and pass sequencing (`collection_pass.py`)

Decision 2 and *Implementation Details → collection_pass.py*.

- [x] **Task 2.1: Contract types** (effort: 2)
  - [x] New `data/kalshi/collection_pass.py`: `PassPhaseName` StrEnum with
        ...
        `EXIT_BY_OUTCOME`).
  - [x] In `events.py`, `SyncEventType` gains `PASS_STARTED = "pass_started"`
  ...
  - [x] Define the `"skipped"` literal once (a module constant) and reference
        it everywhere it is compared.
  - [x] Success: module imports; `PhaseReport.outcome` type never admits
        `"skipped"` into `SyncOutcome`; `to_dict()` output round-trips
        through `json.dumps`.

- [x] **Task 2.2: `classify_pass` and `CollectionPass.run`** (effort: 2)
  - [x] `classify_pass(reports) -> SyncOutcome`, pure: `STORAGE_ABORT` if any
  ...
  - [x] `CollectionPass(run: KalshiRun, phases: Sequence[PassPhase])` with
  ...
  - [x] `PASS_PHASES: tuple[PassPhase, ...]` is declared here as the single
        registration point — filled in Task 3.2 (empty tuple is not a valid
        end state; the section-3 test asserts the first member).
  - [x] Exceptions other than the two caught inside a phase propagate — no
        new `try/except` around `phase.run` in `CollectionPass`.
  - [x] Success: the sequencing and aggregation rules above are visible as
        one loop and one pure function; no I/O other than logging and
        `sink.emit`.

- [x] **Task 2.3: Unit tests with fake phases** (effort: 2)
  - [x] `test/unit/data/kalshi/test_collection_pass.py` with scripted
        ...
        `CollectionPass.run`.
  - [x] `test_events.py`: the two new event types serialize like the
        existing ones (`to_dict` round-trip, valid identifier strings).
  - [x] **Commit**: `feat: add kalshi collection pass contract and sequencing`.

## Section 3: Catalog phase, shared `run_id`, pass events

Decision 3.

- [x] **Task 3.1: `run_id` on `CatalogSync`** (effort: 1)
  - [x] `CatalogSync.__init__` gains `run_id: UUID | None = None`; `None`
        keeps today's fresh `uuid4()` so `sync` is untouched.
  - [x] Success: existing `test_sync_core.py` and `test_data_kalshi.py`
        pass unchanged; a new test in `test_sync_core.py`
        shows a supplied `run_id` appears on every emitted event.

- [x] **Task 3.2: `CatalogPhase` and `PASS_PHASES`** (effort: 2)
  - [x] `CatalogPhase` in `collection_pass.py`: `name = PassPhaseName.CATALOG`;
        ...
        or None, duration_ms=…)`.
  - [x] `PASS_PHASES = (CatalogPhase(),)`.
  - [x] Success: `run_sync`'s classification and `CatalogPhase`'s use the
        same `classify` call; no duplicated outcome mapping.

- [x] **Task 3.3: Unit tests for the catalog phase** (effort: 2)
  - [x] In `test_collection_pass.py`: `PASS_PHASES[0].name is
  ...
  - [x] Full-pass event order with the real phase (Criterion 4): recording
        sink shows `pass_started, run_started, phase_finished ×5,
        run_finished, pass_finished`, one `run_id` throughout.
  - [x] **Commit**: `feat: add kalshi catalog phase with shared run_id and pass events`.

## Section 4: `mt data kalshi pass` CLI

Decision 1 and *Implementation Details → CLI*.

- [x] **Task 4.1: `run_pass` and the `pass` command** (effort: 2)
  - [x] `run_pass(settings, events_file, json_output) -> int` in
  ...
  - [x] Typer command `pass` with `--events-file PATH` and `--json` only (no
        phase options — Decision 1); reuse the existing option objects.
  - [x] Rich summary: one row per phase (`Phase · Outcome · Duration`) then
        the catalog phase's existing summary block (reuse `print_summary`'s
        body for that block — do not duplicate it); `--json` prints
        `PassResult.to_dict()`.
  - [x] Keep the module at ~300 lines: if adding `pass` pushes it over, move
        ...
        both" is now true).
  - [x] Success: `uv run mt data kalshi pass --help` lists exactly the two
        options; `uv run mt data kalshi sync --help` unchanged.

- [x] **Task 4.2: Unit tests for `run_pass`** (effort: 2)
  - [x] In `test_data_kalshi.py` with the same patching seams as the `sync`
        ...
        repeat it.
  - [x] **Commit**: `feat: add mt data kalshi pass command`.

## Section 5: Per-window settled log line (Decision 8, PM-ratified)

- [x] **Task 5.1: INFO line after each watermark write** (effort: 1)
  - [x] In `sync_settled.py`, immediately after a completed window's
  ...
  - [x] Success: a steady-state run logs the line once; the message contains
        no f-string-evaluated SQL or ticker lists.

- [x] **Task 5.2: Unit test on the log record** (effort: 1)
  - [x] In `test_sync_settled.py`, with `caplog` at INFO: a fake source
  ...
  - [x] **Commit**: `feat: log one INFO line per completed kalshi settled window`.
...
- [x] **Task 6.1: `deploy/systemd/mt-kalshi-pass.service`** (effort: 1)
  - [x] Copy `mt-daily-pass.service`; change only `Description=` (the
  ...
  - [x] Success: `diff deploy/systemd/mt-daily-pass.service
...
- [x] **Task 6.2: `deploy/systemd/mt-kalshi-pass.timer`** (effort: 1)
  - [x] Copy `mt-daily-pass.timer`; `[Timer]` becomes the design's block:
  ...
  - [x] Success: `systemd-analyze calendar '*-*-* *:20:00 UTC'` prints
        hourly next-elapse times; the file has no other schedule lines.

- [x] **Task 6.3: `install-production.sh`** (effort: 1)
  - [x] Append `mt-kalshi-pass.service` and `mt-kalshi-pass.timer` to the
        ...
        (Decision 10).
  - [x] Success: `grep -c 'mt-kalshi-pass' deploy/install-production.sh`
        ≥ 3; `bash -n deploy/install-production.sh` clean.

- [x] **Task 6.4: `mt-run` — kinds become data** (effort: 2)
  - [x] `KINDS=(daily minute kalshi)` defined once near the top; `unit_for`
  ...
  - [x] Success: `bash -n deploy/mt-run` clean; the strings `daily` and
        `minute` appear in the script only inside `KINDS` and the usage
        comment.

- [x] **Task 6.5: `mt-run` — root branch forwards every `MT_*` variable** (effort: 2)
  - [x] In `run_production_mt`'s root branch, after sourcing the env file
        ...
        this fixes.
  - [x] Success: with a scratch env file containing `MT_TIMESCALE_DB_URL`,
...
- [x] **Task 6.6: `deploy/manta-trading.env.example`** (effort: 1)
  - [x] Append the three commented lines from the design under "Optional
  ...
  - [x] Success: `grep -c '^#MT_KALSHI' deploy/manta-trading.env.example`
        = 3; the file still contains no uncommented credential.

- [x] **Task 6.7: Consistency test `test/unit/deploy/test_units.py`** (effort: 2)
  - [x] Create `test/unit/deploy/__init__.py` and `test_units.py` (repo root
  ...
  - [x] Both filenames appear inside `install-production.sh`'s `UNITS=( … )`
        block (parse the block, not the whole file); `kalshi` appears inside
        `mt-run`'s `KINDS=( … )`.
  - [x] The CLI side of the drift guard: the `pass` command exists on the
        `kalshi` Typer app (e.g. `CliRunner` `--help` lists `pass`), so the
        unit's `ExecStart` names a real command (Criterion 5).
  - [x] Success: `uv run pytest test/unit/deploy -q` green; deleting the
        kalshi line from `UNITS` makes it fail (check once, restore).

- [x] **Task 6.8: `shellcheck` and commit** (effort: 1)
  - [x] `shellcheck deploy/mt-run deploy/install-production.sh` clean (no CI
        gate for shell — record the command and its empty output in the
        walkthrough refresh, Task 10.3).
  - [x] **Commit**: `feat: add mt-kalshi-pass units, installer entries, mt-run kalshi verb`.

## Section 7: Integration test on `kalshi_db`

- [x] **Task 7.1: `test/integration/test_kalshi_pass.py`** (effort: 3)
  - [x] Same fixtures and fake source as `test_kalshi_sync.py` (and its
  ...
  - [x] Pass ≡ sync: on two fresh databases from the same fixtures, `run_sync`
        and `run_pass` leave identical `sync_state` rows and identical
        per-table row counts.
  - [x] Held lock: while a connection from 262's held-lock test pattern holds
        `SYNC_ADVISORY_LOCK_KEY`, `run_pass` returns 1 (Criterion 2, lock
        case).
  - [x] Events order end-to-end with `--events-file` into `tmp_path`
  ...
  - [x] **Commit**: `test: add kalshi pass integration proofs on kalshi_db`.
...
- [x] **Task 8.1: Quick reference and units** (effort: 1)
  - [x] *What runs by itself*: the `mt-kalshi-pass.service` row (hourly at
  ...
  - [x] *The units*: two rows for the pair; the stop-semantics paragraph
        gains the Kalshi case — `15/TERM` is the normal stop, `9/KILL` is not
        (Decision 5).
  - [x] Success: each new row's command matches Section 6's scripts and
        units verbatim.

- [x] **Task 8.2: Environment file, passes, pausing, rollback** (effort: 1)
  - [x] *Environment file*: the optional Kalshi variables and the PEM
        placement rule (`/etc/manta-trading-kalshi.pem`, `0640
        root:manta-trading`, never under `/home`, installed by hand).
  - [x] *Running the acquisition passes*: `sudo mt-run kalshi`; the
  ...
  - [x] *Pausing a source* and *Rollback*: kalshi units added to the
        examples.
  - [x] Success: the section list in the design's *Runbook 100 changes* is
        fully ticked against the diff.

- [x] **Task 8.3: Add-a-source step and the *Kalshi* subsection** (effort: 2)
  - [x] *Adding a source* checklist: insert between steps 5 and 6 — "add the
        kind to `KINDS` in `deploy/mt-run`".
  - [x] New subsection *Kalshi*: the two status layers (`mt-run status` vs
  ...
  - [x] Update `dateUpdated` in the runbook frontmatter.
  - [x] Success: Criterion 10's list is satisfied; no "B7"-style opaque
        labels — every reference says what it is.

- [x] **Task 8.4: CHANGELOG** (effort: 1)
  - [x] Under `## [Unreleased]`: the pass command and phase contract, the
        ...
        runbook 100 (PM).
  - [x] **Commit**: `docs: add kalshi pass operations to runbook 100 and CHANGELOG`.

## Section 9: Gates and throwaway-database rehearsal (walkthrough steps 1–2)

- [x] **Task 9.1: Full gate pass** (effort: 1)
  - [x] Run every command in walkthrough step 2 (unit tier, kalshi
