...
- [x] **Task 6.1: Append the migration** (effort: 1)
  - [x] Add entry `kalshi_004_catalog_sync_semantics` to `KALSHI_MIGRATIONS`:
  ...
  - [x] Test (`test_kalshi_migrations.py`): `kalshi_004` is in
        ...
        Criterion 8).
  - [x] **Commit**: `feat: add kalshi_004 sync_state comment migration`.

## Section 7: Status reads (`data/kalshi/status.py`)

- [x] **Task 7.1: Status queries and dataclass** (effort: 2)
  - [x] Synchronous psycopg (one short read, the `mt data status` pattern).
  ...
  - [x] Test (`test_kalshi_repository.py` or a sibling integration file):
  ...
  - [x] **Commit**: `feat: add kalshi catalog status reads`.

## Section 8: CLI module (`cli/commands/kalshi.py`)

- [x] **Task 8.1: Exit-code constants and module skeleton** (effort: 1)
  - [x] Create `kalshi_app = typer.Typer(...)`; define `EXIT_OK = 0`,
        ...
        `add_typer` lines.
  - [x] Success: `uv run mt data kalshi --help` lists `sync` and `status`.

- [x] **Task 8.2: Async preflight — connection, session, track, lock** (effort: 3)
  - [x] `open_sync_connection(settings) -> AsyncConnection` in a new
  ...
  - [x] Integration test (`test/integration/test_kalshi_sync.py`, created
...
- [x] **Task 8.3: `mt data kalshi sync`** (effort: 3)
  - [x] Options `--settled-since ISO-8601` (parsed to aware UTC datetime;
  ...
  - [x] Unit test (`test/unit/cli/test_kalshi_commands.py`, typer
  ...
  - [x] **Commit**: `feat: add mt data kalshi sync command with exit codes`.

- [x] **Task 8.4: End-to-end sync scenarios on a throwaway database** (effort: 3)
  - [x] In `test_kalshi_sync.py`, fake source + real repository, one test
  ...
  - [x] Provider abort through the **real** client stack (review F003,
  ...
  - [x] **Commit**: `test: add kalshi sync end-to-end scenarios`.

- [x] **Task 8.5: Storage failure proofs on a throwaway database** (effort: 3)
  - [x] `test_kalshi_sync.py`: (a) a fake-source page carrying one market
  ...
  - [x] **Commit**: `test: prove kalshi sync storage failure taxonomy`.

- [x] **Task 8.6: `mt data kalshi status`** (effort: 2)
  - [x] `--json` option. Opens a sync psycopg connection with the
        ...
        No API call.
  - [x] Unit test: never-synced path exits 0 with the message; `--json`
  ...
  - [x] **Commit**: `feat: add mt data kalshi status command`.

## Section 9: Recorder targets and fixtures

- [x] **Task 9.1: Three new recorder targets** (effort: 2)
  - [x] In `scripts/record_kalshi_fixtures.py` add `markets_by_tickers`
        ...
        `--dry-run`.
  - [x] Run each with `--only` (developer-run, live) and commit the three
        JSON files under `test/fixtures/kalshi/`.
  - [x] Test (`test_fixtures.py`): each new fixture parses into the 261 page
...
- [x] **Task 9.2: Point the fakes at the recorded shapes** (effort: 1)
  - [x] `FakeCatalogSource` (Task 4.1) serves `markets_by_tickers.json` /
  ...
  - [x] **Commit**: `test: record tickers-batch and settled-window kalshi fixtures`.

## Section 10: Final validation and walkthrough

- [x] **Task 10.1: Full validation** (effort: 1)
  - [x] `uv run pytest test/unit -q` green; `uv run python
  ...
  - [x] Walk the design's Success Criteria 1–11 and note where each is
        proven (test name or walkthrough step); Criterion 7's
        unreachable-base-URL clause is Task 8.4's real-client test.

- [x] **Task 10.2: Throwaway-database rehearsal (walkthrough steps 2–5)** (effort: 2)
  - [x] Against a test-cluster throwaway database (runbook 400): apply the
  ...
  - [x] Interrupted drain: run with `--settled-since` a few weeks back,
  ...
  - [x] Live settlement observation (walkthrough step 3): pick an awaiting
        ...
        walkthrough.
  - [x] Drop the throwaway database afterwards (it is one this session
        created).

- [x] **Task 10.3: Refine the walkthrough and close out** (effort: 1)
  - [x] Update the design's *Verification Walkthrough* to the actual
  ...
  - [x] **Commit**: `docs: refine 262 walkthrough post-implementation`.
