...
headroom left after those maxima, not after today's 33 GB. Concrete values belong
in the task breakdown; the constraint is that no test run can make production
slower or push the host into swap.
...
4. The test roles exist on the test cluster and do not gain any new privilege on
   production; production's `pg_roles` set is byte-identical before and after.
...
# 3. Background workers are actually available on the test cluster
...
grep -c "tuple concurrently updated" /tmp/integration-run-*.log   # every count 0
