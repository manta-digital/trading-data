...
- **Decision 2** — first run starts at the trades cutoff and drains ~60 days of live tape forward under the per-pass cap (vs. starting at `now − 24 h` and leaving it to 266).
- **Decision 3** — trades are filtered by the same collection rule (~155 GB/year compressed; alternatives 240 / 41 GB/year), and the settings are renamed `MT_KALSHI_COLLECTION_*` with a loud guard (vs. keeping the candle names). Please also confirm free disk on manta9000.
- **Decision 4** — primary key `(market_ticker, created_time, trade_id)` rather than the slice plan's `(created_time, trade_id)`; `trade_id` stored as `UUID`. On ratification the 260 slice plan's key note is updated to match.
