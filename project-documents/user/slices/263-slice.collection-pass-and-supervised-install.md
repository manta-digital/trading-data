...
8. `systemctl stop` on a running pass ends it with `status=15/TERM`, no `SIGKILL`; the next pass resumes (the watermark and `last_full_sync_at` advance normally). *Amended 20260825:* the unit is left `failed` with `Result=signal`, not `success` — a signal death is a failure to systemd regardless of sender. The resume half is what this criterion is really asserting.
