...
- One INFO line per completed settled window in `sync_settled.py` (the 260 slice plan's note (a); see Decision 8 — the PM may veto).
...
11. The per-window INFO line appears once per completed window in a multi-window run (unit test on the log record; live in the journal) — unless vetoed, in which case this criterion is struck.
