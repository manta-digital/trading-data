...
from repeatedFunctionTimer import RepeatedFnTimer
...
class MarketLink:
    ...
    def addQuoteOnInterval(self, symbol, frequency=Frequency.NONE, n=1, equityType='F', scheduleGroup=None):
        ...
        def f():
            # THIS IS NOT WORKING, no r.content why
        ...
        # Just set a basic interval if that's what was specified.
            ...
            self.timers[schedulingGroup] = RepeatedTimer(interval, f)
    ...
    def removeDuplicates(self, dataFrame, useTransactions=True):
        ...
        # dataFrame = dataFrame.loc[(dataFrame['symbolActive'] != self.dataBuffer['symbolActive']) | (dataFrame['timeQuote'] > self.dataBuffer['timeQuote'])]
        dataFrame = dataFrame.loc[(dataFrame['symbolActive'] != self.dataBuffer['symbolActive']) | (dataFrame[filterColumn] > self.dataBuffer[filterColumn])]
        ...
        if not dataFrame.empty:
            ...
            # hopefully go on...
            self.dataBuffer = self.dataBuffer.append(dataFrame, ignore_index=True)
