...
UNITS=(
  ...
  mt-health.service
  mt-health.timer
  mt-accounting-pass.service
  mt-accounting-pass.timer
  mt-serve.service
)
...
if ! systemctl is-enabled --quiet mt-daily-pass.timer 2>/dev/null; then
  ...
  echo
  echo "Cutover (later, explicit): sudo systemctl enable --now mt-daily-pass.timer mt-minute-pass.timer mt-kalshi-pass.timer mt-health.timer mt-accounting-pass.timer mt-serve.service"
fi
